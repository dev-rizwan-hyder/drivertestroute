...
@section('content')
    <section class="admin-blog-page admin-blog-shell">
        <div class="mx-auto max-w-4xl">
            <div class="mb-8">
                <p class="text-sm font-black uppercase text-cyan-200">Edit Post</p>
                <h1 class="mt-2 text-3xl font-black text-white">{{ $post->title }}</h1>
            </div>
                ...
                <form method="POST" action="{{ route('admin.blog-posts.update', $post) }}" enctype="multipart/form-data">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        ...
                        <label class="block md:col-span-2">
                            ...
                            <input type="text" name="title" value="{{ old('title', $post->title) }}" required class="admin-blog-input mt-1 @error('title') is-invalid @enderror" placeholder="Enter post title">
                            @error('title')
                        ...
                        <label class="block">
                            ...
                            <input type="text" name="read_time" value="{{ old('read_time', $post->read_time) }}" class="admin-blog-input mt-1 @error('read_time') is-invalid @enderror" placeholder="e.g. 5 min read">
                            @error('read_time')
                                <p class="admin-blog-error">{{ $message }}</p>
                            @enderror
                        </label>
                        ...
                        <label class="block md:col-span-2">
                            ...
                            <textarea name="excerpt" required maxlength="500" rows="3" class="admin-blog-input mt-1 @error('excerpt') is-invalid @enderror" placeholder="A short summary of the post (max 500 characters)">{{ old('excerpt', $post->excerpt) }}</textarea>
                            @error('excerpt')
                                <p class="admin-blog-error">{{ $message }}</p>
                            @enderror
                        </label>
                        ...
                        <label class="block md:col-span-2">
                            ...
                            <textarea name="body" required rows="12" class="admin-blog-input mt-1 @error('body') is-invalid @enderror" placeholder="Full post content (HTML supported)">{{ old('body', $post->body) }}</textarea>
                            @error('body')
                                <p class="admin-blog-error">{{ $message }}</p>
                            @enderror
                        </label>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $post->is_published) ? 'checked' : '' }} class="admin-blog-checkbox">
                        ...
                        <label class="block">
                            ...
                            <input type="datetime-local" name="published_at" value="{{ old('published_at', $post->published_at ? $post->published_at->format('Y-m-d\TH:i') : '') }}" class="admin-blog-input mt-1 @error('published_at') is-invalid @enderror">
                            @error('published_at')
                                <p class="admin-blog-error">{{ $message }}</p>
                            @enderror
                        </label>
                    ...
                    <div class="mt-8 flex justify-end gap-3">
                        ...
                        <button type="submit" class="admin-blog-button admin-blog-primary">Update Post</button>
                    </div>
                </form>
            </div>
        </div>
